...
df.camel.ms <- read_tsv("input/masked_superstrings_properties__large_d.tsv")
...
df.mem0 <- read_tsv("input/camel_memtime__large_d.tsv")
df.mem <- df.mem0 %>%
    transmute(
        pref0 = pref,
...
df %>%
    write_tsv("all_data__larged.tsv",  na = "na")
